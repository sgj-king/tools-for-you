...
export async function POST(request: NextRequest) {
  ...
  try {
    payload = await parseJsonBody<{ email?: string; password?: string }>(request, 16 * 1024);
  } catch (error) {
  ...
  const response = NextResponse.json({
    data: {
      sessionUser,
      redirectTo: sessionUser.role === "platform_super_admin" || sessionUser.role === "ops_admin" ? "/admin" : "/console",
